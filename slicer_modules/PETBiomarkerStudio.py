...
from lib.quantification.biomarker_batch import (
    ...
    resolve_batch_segment_file as _lib_resolve_batch_segment_file,
...
class PETBiomarkerStudioWidget(ScriptedLoadableModuleWidget):
    def setup(self):
        ...
        self.radResampleCheckBox.setChecked(True)
        self.radResampleCheckBox.setToolTip(
            "Recommended for large ROIs (e.g. visceral fat). "
            "Native-resolution GLCM/GLSZM often runs out of memory."
        )
    ...
    # Therefore the visualization reads the exported Quantification sheet and keeps the
    ...
    def setupVisualizationTab(self, layout):
        ...
        self.visualizationExcelEdit.setToolTip(
            "Excel file generated by PET Biomarker Studio. The Quantification sheet is used."
        )

        self.visualizationRefreshButton = qt.QPushButton("Load / Refresh")
        self.visualizationRefreshButton.setToolTip(
            "Reload the Quantification sheet and rebuild the metric and segment selections."
        )
    ...
    def _setupQuantifySubTab(self, layout):
        ...
        self.batchRadResampleCheckBox.setChecked(True)
        self.batchRadResampleCheckBox.setToolTip(
            "Recommended for large ROIs (e.g. visceral fat). "
            "Native-resolution GLCM/GLSZM often runs out of memory."
        )
    ...
    def _populateBatchSegTable(self, stems):
        ...
        processed_bases = {
            s[: -len("_processed")]
            for s in stems
            if s.lower().endswith("_processed")
        }
        self.batchSegTable.setRowCount(len(stems))
        for r, stem in enumerate(stems):
            includeCheck = qt.QCheckBox()
            is_aux = self.logic.isAuxiliarySegmentStem(stem)
            is_processed = stem.lower().endswith("_processed")
            raw_has_processed = (not is_processed) and (stem in processed_bases)
            # Prefer *_processed when both raw and processed exist
            includeCheck.setChecked(
                (not is_aux)
                and (
                    is_processed
                    or (not raw_has_processed)
                )
            )
            cellW = qt.QWidget()
            cellL = qt.QHBoxLayout(cellW)
...
class PETBiomarkerStudioLogic(ScriptedLoadableModuleLogic):
   ...
   # ----- Radiomics calculation related functions -----
    ...
    def loadMetricRowsFromExcel(self, excelPath):
        ...
        from lib.quantification.biomarker_batch import quantification_sheet_name

        workbook = openpyxl.load_workbook(
            excelPath, read_only=True, data_only=True
        )
        try:
            sheet = quantification_sheet_name(workbook.sheetnames)
            if sheet is None:
                raise ValueError(
                    f"The Excel file has no 'Quantification' (or legacy 'Data') sheet: {excelPath}"
                )
   ...
   # ----- Excel output related functions -----
    def saveMetricRowsToExcel(self, rows, outputFile, append=True):
        """Write Quantification (+ Radiomics when present) via shared lib helper."""
    ...
    def _findBatchSegmentFile(self, segDir, stem, preferProcessed=True):
        """Prefer ``{stem}_processed`` when present (quantification default)."""
        return _lib_resolve_batch_segment_file(
            segDir, stem, prefer_processed=preferProcessed
        )
    ...
    def runBatchQuantification(self, root, segmentSelections, metricsOptions,
        ...
        """Iterate subjects, quantify the selected segment files, write
        Quantification (+ Radiomics when enabled) and Summary sheets.
        Computation reuses runPetIndic / runRadiomics so single
            ...
            try:
                ...
                for stem, label in segmentSelections:
                    ...
                    if segPath is None:
                        ...
                        continue
                    print(f"  [mask] {label} ← {os.path.basename(segPath)}")
    ...
    def saveBatchRowsToExcel(self, rows, outputFile, append=True):
        """Write Quantification (+ Radiomics) and Summary sheets."""
        return _lib_save_batch_rows_to_excel(rows, outputFile, append=append)
    ...
    def runBatchPrepare(self, root, organStems, steps, auxConfig, params,
        ...
        for idx, segFolder in enumerate(subjectDirs):
            ...
            try:
                ...
                anyOrgan = False
                for stem in organStems:
                    # Ablation must start from the raw organ mask, not *_processed
                    organPath = _lib_find_batch_segment_file(segDir, stem)
